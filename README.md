...
Программа для работы с банковскими транзакциями с расширенной функциональностью.

## Новая функциональность

### 🔍 Поиск операций с использованием регулярных выражений
- Реализована функция `process_bank_search()` в модуле `operations.py`
- Использует библиотеку `re` для поиска по описанию операций
- Поддерживает регистронезависимый поиск
- Ищет целые слова с учетом границ слов

### 📊 Подсчет операций по категориям
- Реализована функция `process_bank_operations()` в модуле `operations.py`
- Использует `Counter` из библиотеки `collections` для эффективного подсчета
- Возвращает статистику по заданным категориям операций

### 🎲 Случайная выборка операций
- Добавлена функция `get_random_operation_sample()` в `main.py`
- Использует библиотеку `random` для выбора случайных операций
- Позволяет просмотреть репрезентативную выборку данных

### 📈 Статистика по категориям
- Новая функция `show_category_statistics()` в основном интерфейсе
- Показывает количество операций по выбранным категориям

## Структура проекта
bank-operations-widget/
├── data/ # Файлы с данными операций
│ ├── operations.json # Данные в JSON формате
│ ├── transactions.csv # Данные в CSV формате
│ └── operations.xlsx # Данные в Excel формате
├── src/ # Исходный код
│ ├── main.py # Основной модуль с логикой программы
│ └── operations.py # Модуль с функциями обработки операций
├── tests/ # Тесты
│ └── test_operations.py # Тесты для новых функций
└── requirements.txt # Зависимости проекта

## Используемые библиотеки

- **re** - для работы с регулярными выражениями при поиске операций
- **collections.Counter** - для эффективного подсчета операций по категориям
- **random** - для создания случайных выборок операций
